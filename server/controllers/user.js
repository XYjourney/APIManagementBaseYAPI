...
class userController extends baseController {
  ...
  async loginByToken(ctx) {
    try {
      ...
      console.log("in loginBytoken");
      let login = await this.handleThirdLogin(ret.email, ret.username, ret.thirdId);
      if (login === true) {
        yapi.commons.log('login success');
        ctx.body = yapi.commons.resReturn(null, 200, true);
      } else {
        ctx.body = yapi.commons.resReturn(null, 401, false);
      }
    } catch (e) {
  ...
  // 处理第三方登录
  async handleThirdLogin(email, username, thirdId) {
    ...
    try {
      ...
      if (!user || !user._id) {
        ...
        data = {
          username: username,
          password: yapi.commons.generatePassword(thirdId, passsalt),
          email: email,
          passsalt: passsalt,
          role: 'member',
        ...
        if(yapi.commons.generatePassword(thirdId, user.passsalt) == user.password){      
          this.setLoginCookie(user._id, user.passsalt);
          return true;
        }
      }
